-- Manual Database Setup for Supabase Dashboard
...
DROP MATERIALIZED VIEW IF EXISTS leaderboard_daily CASCADE;
...
CREATE INDEX IF NOT EXISTS idx_analyses_ticker_observed ON analyses (ticker_id, observed_at DESC);
CREATE INDEX IF NOT EXISTS idx_analyses_options_hash ON analyses (options_hash);
CREATE INDEX IF NOT EXISTS idx_analyses_observed_at ON analyses (observed_at DESC);

-- Query-specific indexes
CREATE INDEX IF NOT EXISTS idx_analyses_score_total ON analyses (score_total DESC) WHERE score_total IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_analyses_forward_yield ON analyses (forward_yield DESC) WHERE forward_yield IS NOT NULL;
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_analyses_daily_unique ON analyses (
  ticker_id, 
  options_hash, 
...
-- Leaderboard index
CREATE INDEX IF NOT EXISTS idx_leaderboard_date_rank ON leaderboard_daily (as_of_date DESC, rank);
