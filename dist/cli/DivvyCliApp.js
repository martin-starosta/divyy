...
export class DivvyCliApp {
    ...
    async run() {
        try {
            ...
            if (options.verbose) {
                const qualityReport = FallbackDataProvider.assessDataQuality(true, // has price (we got this far)
                analysis.annualDividends.length, this.countFundamentalFields(analysis.fundamentals), 5 // total fundamental fields
                );
                console.log(ErrorFormatter.formatDataQualityReport(qualityReport.score, qualityReport.level, qualityReport.recommendations));
            }
