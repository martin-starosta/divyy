...
import { DividendEvent, Fundamentals } from "../models/StockData.js";
import { RetryHandler } from "../utils/RetryHandler.js";
import { DataQualityChecker } from "../validation/DataQualityChecker.js";
import { NetworkError, DataSourceError, TickerNotFoundError, RateLimitError } from "../errors/DivvyErrors.js";
export class YahooFinanceService {
    async getQuote(ticker) {
        try {
            const quote = await RetryHandler.withRetry(async () => {
    ...
    async getDividendEvents(ticker, years = 15) {
        try {
            ...
            const chart = await RetryHandler.withRetry(async () => {
    ...
    async getFundamentals(ticker, years = 15) {
        ...
        try {
            const fundamentalsData = await RetryHandler.withRetry(async () => {
        ...
        if (!hasData) {
            try {
                const quoteSummary = await RetryHandler.withRetry(async () => {
    ...
    handleYahooError(error, operation, ticker) {
        ...
        if (error?.message?.includes('429') || error?.message?.includes('rate limit')) {
            return new RateLimitError(`Yahoo Finance rate limit exceeded for ${operation}`, 60000 // 1 minute retry delay
            );
        }
        if (error?.code === 'ENOTFOUND' || error?.code === 'ECONNREFUSED') {
            return new NetworkError(`Network error accessing Yahoo Finance for ${operation}: ${error.message}`);
        }
        if (error?.message?.includes('timeout')) {
            return new NetworkError(`Timeout error accessing Yahoo Finance for ${operation}`);
        }
        ...
        return new DataSourceError(`Yahoo Finance ${operation} failed: ${error.message}`, 'yahoo', true // Most Yahoo errors are retryable
        );
    }
